...
func (p *Proxy) newProxyReverseProxy(frontend, backend *url.URL) *httputil.ReverseProxy {
	...
	//定义一个NotFoundErr
	NotFoundErr := errors.New("response is 404, need to redirece")
	return &httputil.ReverseProxy{
		...
		ErrorHandler: func(writer http.ResponseWriter, request *http.Request, err error) {
			//判断Error是否为NotFoundError，是的话进行前端服务的转发，重新修改writer
...
func (p *Proxy) restartFrontend() error{
	...
	//npm run serve 是控制台挂起模式，使用go routine启动
	err=cmd.Start()
...
func (p *Proxy) startProxy(startFrontend, startBackend bool) error{
	...
	//设置反向代理
	proxtReverse := p.newProxyReverseProxy(frontendURL,backendURL)
	proxyServer:=&http.Server{
		Addr: "127.0.0.1:"+p.devConfig.Port,
		Handler: proxtReverse,
	}
